...
export async function DELETE(request:Request){try{const {date,id}=await request.json();dateSchema.parse(date);return stateResponse(await updateState(state=>{const day=state.days[date];if(day)day.weights=(day.weights??[]).filter(entry=>entry.id!==id)}))}catch(error){return NextResponse.json({error:error instanceof Error?error.message:String(error)},{status:400})}}
