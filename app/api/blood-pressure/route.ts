...
export async function DELETE(request:Request){try{const {date,id}=await request.json();dateSchema.parse(date);if(typeof id!=="string")throw new Error("Invalid id");return NextResponse.json(await updateState(state=>{const day=state.days[date];if(day)day.bloodPressures=(day.bloodPressures??[]).filter(entry=>entry.id!==id)}))}catch(error){return NextResponse.json({error:error instanceof Error?error.message:String(error)},{status:400})}}
