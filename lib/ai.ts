...
export function foodAnalysisPrompt(text:string,schema:unknown){
 const visualSizeInstruction=`For food photos, before estimating consumed grams, draw tight bounding boxes in your internal visual reasoning around each visible food item or distinct food portion and each usable reference object. Label each box with its image and object, and represent it as [x_min, y_min, x_max, y_max], normalized from 0 to 1 relative to that image's width and height, with the origin at the top-left. Bound the food itself separately from its plate or container; do not count empty space as food.
Use a reference object with known or user-provided physical dimensions whenever possible. If using a plate, takeout container, packaging, or utensil of unknown size, treat its dimensions as an uncertain assumption, not a standard measurement. Compare the food and reference boxes within the same image and at a similar distance from the camera. Estimate physical width and length from corresponding box dimensions: food size = reference physical size × food box span / reference box span. Account for rotation, perspective, foreshortening, and occlusion; do not compare raw box spans across different images. Use multiple views to refine the same portion without counting it twice.
Use the scaled food dimensions, estimated thickness or fill depth, shape, and plausible food density to estimate volume and then grams for the amount consumed. A two-dimensional bounding box alone does not establish volume or mass. Keep uncertain depth, hidden food, and reference dimensions reflected in confidence and total_range. If there is no usable scale reference, fall back to the description, visible quantity, or typical portion sizes with lower confidence; do not invent a reference object or precise measurements. Skip bounding boxes for text-only inputs or images that show only labels, receipts, menus, or app screenshots. Applicable displayed nutrition values remain authoritative over visual estimates. Keep the boxes and measurement reasoning internal; return only the requested JSON fields.`;
